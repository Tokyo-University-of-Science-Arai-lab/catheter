...
def _key_score(key: str, combined: str, *, is_date: bool = False) -> float:
    ...
    return float(fuzz.partial_ratio(_normalize(key), _normalize(combined)))
...
def match_text_to_mask_main(
    ...
    *,
    ...
    confident = (top >= CONFIDENT_SCORE) and (margin >= CONFIDENT_MARGIN)
    ...
    try:
        ...
        (shot_dir / "multikey_match_debug.json").write_text(
            json.dumps({
                ...
                "confident_rule": f"score>={CONFIDENT_SCORE} and margin>={CONFIDENT_MARGIN}",
                "per_mask": [
                    {
                        "mask": f"mask_{i + 1}",
