...
def _residual_policy_has_a(mode: str) -> bool:
    return str(mode).upper() in {"A", "AB", "AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"}


def _residual_policy_has_b(mode: str) -> bool:
    return str(mode).upper() in {"B", "AB", "AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"}


def _residual_policy_a_variant(mode: str) -> str:
    mode = str(mode).upper()
    if mode in {"AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"}:
        return "a_tuned_v2"
...
def _evaluate_residual_policy_ab_safe_interaction(
    *,
    ...
    out = {
        "mode": mode,
        "enabled": mode in {"AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"},
    ...
    if mode not in {"AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"}:
...
def _evaluate_residual_policy_b_width(
    *,
    ...
    tuned_v2_reject_reasons = []
    tuned_v2_kept = False
    if mode in {"AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"} and reject_reasons and robust_width_mm is not None:
    ...
    v3_block_reasons = []
    v3_blocked = False
    v3_enabled = mode == "AB_SAFE_A_TUNED_V3"
    if v3_enabled:
        out["v3_enabled"] = True
...
def _run_recognition_core_like_offline(
    *,
    ...
    residual_policy_interaction_info = {
        ...
        "enabled": residual_policy_mode in {"AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3"},
        "A_then_B_overwrite_prevented": False,
        "A_after_width_plausible": False,
        "A_after_width_sensitivity": None,
        "notes": ["not evaluated"],
