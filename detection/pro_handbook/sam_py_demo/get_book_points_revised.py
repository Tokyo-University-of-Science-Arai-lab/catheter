...
def refine_mask_by_spine_column_length_after_depth(
    ...
    *,
    ...
    force_remove_side: str | int | None = None,
    forced_side_info: dict | None = None,
    return_info: bool = False,
):
    """
    ...
    info = {
        ...
        "force_remove_side": None if force_remove_side is None else str(force_remove_side),
        "forced_side_info": forced_side_info if isinstance(forced_side_info, dict) else None,
    ...
    forced_sign = _side_name_to_sign(force_remove_side)
    forced_side_used = False
    side_decision_source = "outside_candidate_points"

    if bool(one_sided_removal) and forced_sign in (-1, +1):
        # Depth prefilter で先に削れた側を，後段でも削るべき側として固定する。
        # これにより，Depth後に残った外側候補数だけを見て反対側を削る誤判定を避ける。
        if forced_sign > 0:
        ...
        forced_side_used = True
        side_decision_ambiguous = False
        side_decision_source = "forced_from_depth_prefilter_removed_side"
    elif bool(one_sided_removal) and total_outside_points >= int(one_sided_min_candidate_points):
    ...
    info.update({
        ...
        "side_decision_source": str(side_decision_source),
        "forced_remove_side_used": bool(forced_side_used),
        "forced_remove_side": None if force_remove_side is None else str(force_remove_side),
        "forced_side_info": forced_side_info if isinstance(forced_side_info, dict) else None,
...
def _draw_target_point_on_final_png(
    shot_dir: Path,
    color_np: np.ndarray,
    mask01: np.ndarray,
    depth_masked: np.ndarray,
    ...
    *,
    ...
    """
    final.png に目標点を小さな赤点で追記して保存する。

    注意:
      - 領域境界を見やすくするため，黄色輪郭は描画しない。
      - 緑の重ね塗り + 赤い目標点だけを描画する。
      - 目標点が投影できない場合もfinal.png自体は緑overlayとして保存する。
    """
    shot_dir = Path(shot_dir)
    color_np = np.asarray(color_np)
    mask01 = (np.asarray(mask01) > 0).astype(np.uint8)
    ...
    info = {
        ...
        "reason": "not started",
        "final_png": str(shot_dir / "final.png"),
        "target_radius_px": int(radius_px),
        "target_color_bgr": [0, 0, 255],
        "note": "final.png is green region overlay only, with a small red target point when projection is valid; yellow contour is intentionally disabled.",
    }

    try:
        target = np.asarray(target_point_m, dtype=np.float64).reshape(-1)
        if target.size < 3:
            info["reason"] = "target point has fewer than 3 values"
        else:
            x, y, z = float(target[0]), float(target[1]), float(target[2])
            info["target_point_m"] = [x, y, z]
            if not np.isfinite(z) or z <= 1e-9:
                info["reason"] = f"invalid target z: {z}"
            else:
                u = int(round(float(intr.fx) * x / z + float(intr.ppx)))
                v = int(round(float(intr.fy) * y / z + float(intr.ppy)))
                h, w = color_np.shape[:2]
                info["target_pixel_uv"] = [int(u), int(v)]
                info["image_size_wh"] = [int(w), int(h)]
                if 0 <= u < w and 0 <= v < h:
                    cv2.circle(blended, (u, v), int(radius_px), (0, 0, 255), -1, cv2.LINE_AA)
                    info.update({"used": True, "reason": "ok"})
                else:
                    info["reason"] = "projected target point is outside image"
    except Exception as e:
        info["reason"] = f"failed to draw target point: {e}"

    cv2.imwrite(str(shot_dir / "final.png"), blended)
    save_json(shot_dir / f"{stem}_final_target_overlay_log.json", info)
    return info
...
def apply_one_sided_short_column_prune_after_final(
    ...
    *,
    ...
    force_removed_side: str | int | None = None,
    return_info: bool = False,
):
    """
    ...
    info = {
        ...
        "valid_count_before": int(valid_count_before),
        "force_removed_side": None if force_removed_side is None else str(force_removed_side),
    }
    ...
    forced_sign = _side_name_to_sign(force_removed_side)
    if forced_sign is not None:
        removed_side_sign = int(forced_sign)
        removed_side_source = "forced_from_column_info_remove_side"
        removed_side_detail = {"force_removed_side": str(force_removed_side)}

    if removed_side_sign is None and side_reference_mask_before is not None and side_reference_mask_after is not None:
...
def _depth_prefilter_side_lock_enabled(mode: str) -> bool:
    """
    Depth prefilter で先に削れた側を，後段の remove_side として優先する実験モード。

    背景:
      00_selected -> 01_after_depth_prefilter で側面だけがきれいに落ちるケースでは，
      その削れた側こそ「削るべき側」として信頼できる。
      逆に，Depth後の残存点だけで外側候補数を比較すると，すでに側面が消えた側の
      候補数が少なくなり，反対側を remove_side と誤判定することがある。
    """
    return str(mode).upper() == "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"


def infer_remove_side_from_depth_prefilter_removed_pixels(
    *,
    selected_mask01_before_depth: np.ndarray,
    mask01_after_depth_prefilter: np.ndarray,
    refine_info: dict | None,
    image_shape: tuple[int, int],
    min_removed_pixels: int = 300,
    dominant_fraction_min: float = 0.70,
    dominant_ratio_min: float = 1.50,
):
    """
    00_selected と 01_after_depth_prefilter を比較し，Depthでより多く削れた側を remove_side として推定する。

    ルール:
      - 左右は使わず，OCR/SAM軸から得た画像 t 座標の positive/negative だけで判定する。
      - 片側だけ大きく削れている場合は，その側を remove_side にする。
      - 両側削れている場合も，より多く削れた側を remove_side として採用する。
      - ただし差が小さい場合は ambiguous とし，従来の column refine 判定へフォールバックする。
    """
    before = (np.asarray(selected_mask01_before_depth) > 0).astype(np.uint8)
    after = (np.asarray(mask01_after_depth_prefilter) > 0).astype(np.uint8)
    info = {
        "used": False,
        "reason": "not initialized",
        "algorithm": "infer_remove_side_from_depth_prefilter_removed_pixels",
        "min_removed_pixels": int(min_removed_pixels),
        "dominant_fraction_min": float(dominant_fraction_min),
        "dominant_ratio_min": float(dominant_ratio_min),
        "before_area_px": int(before.sum()),
        "after_area_px": int(after.sum()),
    }
    try:
        axis, normal, center, axis_source = _stage_axis_center_from_infos(
            before,
            refine_info if isinstance(refine_info, dict) else {},
            {},
        )
    except Exception as e:
        info["reason"] = f"axis extraction failed: {e}"
        return None, info

    if axis is None or normal is None or center is None:
        info["reason"] = "axis/normal/center unavailable"
        return None, info

    removed = (before > 0) & ~(after > 0)
    ys, xs, t = _t_values_for_mask_pixels(removed.astype(np.uint8), normal, center)
    removed_count = int(xs.size)
    neg = int(np.count_nonzero(t < 0)) if xs.size else 0
    pos = int(np.count_nonzero(t > 0)) if xs.size else 0
    center_count = int(removed_count - neg - pos)
    max_side = max(neg, pos)
    min_side = min(neg, pos)
    dominant_fraction = float(max_side / max(removed_count, 1))
    dominant_ratio = float(max_side / max(min_side, 1))
    ...
    info.update({
        ...
        "center": [float(center[0]), float(center[1])],
        "removed_count": int(removed_count),
        "removed_negative_count": int(neg),
        "removed_positive_count": int(pos),
        "removed_center_count": int(center_count),
        "dominant_fraction": float(dominant_fraction),
        "dominant_ratio": float(dominant_ratio),
        "note": "positive/negative are image-t signs. Display-flipped left/right is intentionally ignored.",
    })

    if removed_count < int(min_removed_pixels):
        info["reason"] = f"too few depth-removed pixels: {removed_count}"
        return None, info
    if max_side <= 0:
        info["reason"] = "no positive/negative removed pixels"
        return None, info

    # 片側が明確に多い場合のみ採用する。
    # 両側削れていても，より多い側が十分支配的なら採用する。
    if dominant_fraction < float(dominant_fraction_min) and dominant_ratio < float(dominant_ratio_min):
        info["reason"] = "depth-removed side is ambiguous"
        info["ambiguous"] = True
        return None, info
    ...
    state = {
        "remove_side": remove_side,
        "protected_side": protected_side,
        "remove_sign": int(remove_sign),
        "protected_sign": int(protected_sign),
        "axis": np.asarray(axis, dtype=np.float64).reshape(2),
        "normal": np.asarray(normal, dtype=np.float64).reshape(2),
        "center": np.asarray(center, dtype=np.float64).reshape(2),
    }
    info.update({
        "used": True,
        "reason": "ok",
        "side_decision_source": "depth_prefilter_removed_pixels",
        "remove_side": remove_side,
        "protected_side": protected_side,
        "remove_sign": int(remove_sign),
        "protected_sign": int(protected_sign),
    ...
    if xs.size:
        info.update({
            "removed_t_min": float(np.min(t)),
            "removed_t_p05": float(np.percentile(t, 5.0)),
            "removed_t_p50": float(np.percentile(t, 50.0)),
            "removed_t_p95": float(np.percentile(t, 95.0)),
            "removed_t_max": float(np.max(t)),
        })
    return state, info

def _one_sided_consistency_restore_enabled(mode: str) -> bool:
    """
    片側削除の一貫性を強制する実験モード。

    目的:
      - column refine で remove_side / protected_side が決まった後は，
        protected_side 側の点を後段処理で削らない。
      - その判断以前に Depth prefilter などで落ちた protected_side 側の点は，
        初期選択SAMマスクとの差分から条件付きで復活させる。

    既存モードは変えず，新モードだけで有効にする。
    """
    return str(mode).upper() in {
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE",
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2",
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3",
    }


def _side_name_to_sign(side_name):
    if side_name is None:
        return None
    s = str(side_name).strip().lower()
    if s in {"positive", "positive_t", "+", "+1", "1"}:
        return +1
    if s in {"negative", "negative_t", "-", "-1"}:
        return -1
    return None


def _extract_one_sided_state_from_column_info(column_info: dict | None, *, require_used: bool = True):
    """column_info から画像t座標系の remove/protected side と axis/normal/center を取り出す。"""
    info = column_info if isinstance(column_info, dict) else {}
    out = {
        "used": False,
        "reason": "not initialized",
        "remove_side": info.get("remove_side"),
        "protected_side": info.get("protected_side"),
        "one_sided_removal_used": bool(info.get("one_sided_removal_used", False)),
    }
    if require_used and not bool(info.get("one_sided_removal_used", False)):
        out["reason"] = "one_sided_removal was not used"
        return None, out

    remove_sign = _side_name_to_sign(info.get("remove_side"))
    protected_sign = _side_name_to_sign(info.get("protected_side"))
    if remove_sign is None or protected_sign is None:
        out["reason"] = "remove_side/protected_side is unavailable"
        return None, out
    if int(remove_sign) == int(protected_sign):
        out["reason"] = "remove_side and protected_side have the same sign"
        return None, out

    try:
        axis = np.asarray(info.get("axis"), dtype=np.float64).reshape(2)
        normal = np.asarray(info.get("normal"), dtype=np.float64).reshape(2)
        center = np.asarray(info.get("center"), dtype=np.float64).reshape(2)
    except Exception:
        out["reason"] = "axis/normal/center is unavailable"
        return None, out

    an = float(np.linalg.norm(axis))
    nn = float(np.linalg.norm(normal))
    if an < 1e-9 or nn < 1e-9:
        out["reason"] = "axis/normal norm is too small"
        return None, out
    axis = axis / an
    normal = normal / nn

    state = {
        "remove_side": "positive" if int(remove_sign) > 0 else "negative",
        "protected_side": "positive" if int(protected_sign) > 0 else "negative",
        "remove_sign": int(remove_sign),
        "protected_sign": int(protected_sign),
        "axis": axis,
        "normal": normal,
...
def _t_values_for_mask_pixels(mask01: np.ndarray, normal: np.ndarray, center: np.ndarray):
    mask = (np.asarray(mask01) > 0)
    ys, xs = np.where(mask)
    if xs.size <= 0:
        return ys, xs, np.asarray([], dtype=np.float64)
    pts = np.stack([xs.astype(np.float64), ys.astype(np.float64)], axis=1)
    t = (pts - np.asarray(center, dtype=np.float64).reshape(1, 2)) @ np.asarray(normal, dtype=np.float64).reshape(2)
    return ys, xs, t


def _restore_pixels_with_depth(
    *,
    mask01: np.ndarray,
    depth_masked: np.ndarray,
    restore_mask01: np.ndarray,
    depth_source: np.ndarray | None = None,
    fill_depth_by_current_row_median: bool = True,
):
    """restore_mask01 の画素を mask/depth に戻す。depth_sourceが無効なら現在maskの行中央値で補う。"""
    mask_out = (np.asarray(mask01) > 0).astype(np.uint8).copy()
    depth_out = np.asarray(depth_masked).copy()
    restore = (np.asarray(restore_mask01) > 0)
    restore &= (mask_out == 0)
    restore_count = int(np.count_nonzero(restore))
    info = {
        "restore_count": int(restore_count),
        "depth_from_source_count": 0,
        "depth_from_row_median_count": 0,
        "depth_from_global_median_count": 0,
    }
    if restore_count <= 0:
        return mask_out, depth_out, info

    mask_out[restore] = 1
    depth_src = None if depth_source is None else np.asarray(depth_source)

    current_valid = (np.asarray(mask01) > 0) & (np.asarray(depth_masked) > 0)
    current_values = np.asarray(depth_masked)[current_valid]
    current_values = current_values[current_values > 0]
    global_med = int(np.median(current_values)) if current_values.size > 0 else 0

    ys, xs = np.where(restore)
    for y in np.unique(ys):
        idx = np.where(ys == y)[0]
        yy = ys[idx]
        xx = xs[idx]
        assigned = np.zeros((idx.size,), dtype=bool)

        if depth_src is not None:
            vals_src = depth_src[yy, xx]
            good_src = vals_src > 0
            if np.any(good_src):
                depth_out[yy[good_src], xx[good_src]] = vals_src[good_src].astype(depth_out.dtype, copy=False)
                assigned[good_src] = True
                info["depth_from_source_count"] += int(np.count_nonzero(good_src))

        if fill_depth_by_current_row_median and np.any(~assigned):
            row_vals = np.asarray(depth_masked)[int(y), :]
            row_mask = current_valid[int(y), :]
            row_vals = row_vals[row_mask & (row_vals > 0)]
            if row_vals.size > 0:
                z = int(np.median(row_vals))
                yy2 = yy[~assigned]
                xx2 = xx[~assigned]
                depth_out[yy2, xx2] = np.asarray(z, dtype=depth_out.dtype)
                info["depth_from_row_median_count"] += int(yy2.size)
                assigned[~assigned] = True

        if np.any(~assigned) and global_med > 0:
            yy2 = yy[~assigned]
            xx2 = xx[~assigned]
            depth_out[yy2, xx2] = np.asarray(global_med, dtype=depth_out.dtype)
            info["depth_from_global_median_count"] += int(yy2.size)

    depth_out[mask_out == 0] = 0
    return mask_out.astype(np.uint8), depth_out, info



def restore_protected_side_from_selected_mask_diff(
    *,
    current_mask01: np.ndarray,
    current_depth_masked: np.ndarray,
    selected_mask01_before_depth: np.ndarray,
    raw_depth_u16: np.ndarray,
    column_info: dict | None,
    shot_dir: str | Path | None = None,
    color_np: np.ndarray | None = None,
    stem: str = "protected_side_restore_from_selected_diff",
    s_margin_px: float = 18.0,
    max_restore_extension_px: float = 14.0,
    inner_overlap_px: float = 2.0,
    max_width_growth_px: float = 16.0,
    max_restore_area_ratio: float = 0.22,
    max_restore_pixels: int = 6500,
    return_info: bool = True,
):
    """
    side判定以前に落ちた protected_side 側の画素を，初期選択SAMマスクとの差分から復活する。

    重要な設計:
      - 画像を上下反転して表示しても，ここでは左右ではなく t 符号だけで判定する。
      - rgb_mask*_selected*_overlay.jpg は可視化画像なので本流処理には使わず，その元データである
        selected_mask01_before_depth を復活上限候補として使う。
      - selected mask 全体を戻すと側面・前面も戻って幅が巨大化するため，現在maskの protected 側境界に
        近い帯だけを戻す。
      - remove_side 側は絶対に復活候補にしない。
    """
    mask_cur = (np.asarray(current_mask01) > 0).astype(np.uint8)
    depth_cur = np.asarray(current_depth_masked).copy()
    selected = (np.asarray(selected_mask01_before_depth) > 0).astype(np.uint8)
    state, state_info = _extract_one_sided_state_from_column_info(column_info, require_used=True)
    info = {
        "enabled": True,
        "used": False,
        "reason": "not initialized",
        "algorithm": "restore_protected_side_from_selected_mask_diff_limited_edge_band",
        "side_state": state_info,
        "s_margin_px": float(s_margin_px),
        "max_restore_extension_px": float(max_restore_extension_px),
        "inner_overlap_px": float(inner_overlap_px),
        "max_width_growth_px": float(max_width_growth_px),
        "max_restore_area_ratio": float(max_restore_area_ratio),
        "max_restore_pixels": int(max_restore_pixels),
        "selected_area_px": int(selected.sum()),
        "area_before_restore": int(mask_cur.sum()),
        "note": "Restore is based on selected-mask difference, but restricted to the protected-side edge band in t coordinates. Display-flipped left/right is not used.",
    }
    if state is None:
        info["reason"] = "side state unavailable"
        return (mask_cur, depth_cur, info) if return_info else (mask_cur, depth_cur)

    axis = state["axis"]
    normal = state["normal"]
    center = state["center"]
    protected_sign = int(state["protected_sign"])
    remove_sign = int(state["remove_sign"])

    # 現在残っているmaskの t/s 分布を基準にする。
    ys_c, xs_c = np.where(mask_cur > 0)
    if xs_c.size < 20:
        info["reason"] = "too few current pixels"
        return (mask_cur, depth_cur, info) if return_info else (mask_cur, depth_cur)
    pts_c = np.stack([xs_c.astype(np.float64), ys_c.astype(np.float64)], axis=1)
    rel_c = pts_c - center.reshape(1, 2)
    s_c = rel_c @ axis
    t_c = rel_c @ normal

    try:
        s_min_keep, s_max_keep = np.percentile(s_c, [1.0, 99.0])
        s_min_keep = float(s_min_keep - float(s_margin_px))
        s_max_keep = float(s_max_keep + float(s_margin_px))
    except Exception:
        s_min_keep = float(np.min(s_c) - float(s_margin_px))
        s_max_keep = float(np.max(s_c) + float(s_margin_px))

    cur_t_min = float(np.min(t_c))
    cur_t_max = float(np.max(t_c))
    cur_t_p02 = float(np.percentile(t_c, 2.0))
    cur_t_p98 = float(np.percentile(t_c, 98.0))
    cur_width_p02p98 = float(max(0.0, cur_t_p98 - cur_t_p02))

    # protected側の現在境界から外側にだけ，最大 max_restore_extension_px まで戻す。
    # protected_sign=-1: t負側がprotectedなので，現在の最小tより少し外側だけ。
    # protected_sign=+1: t正側がprotectedなので，現在の最大tより少し外側だけ。
    if protected_sign < 0:
        edge_t = cur_t_min
        t_band_min = float(edge_t - float(max_restore_extension_px))
        t_band_max = float(edge_t + float(inner_overlap_px))
    else:
        edge_t = cur_t_max
        t_band_min = float(edge_t - float(inner_overlap_px))
        t_band_max = float(edge_t + float(max_restore_extension_px))

    candidate = (selected > 0) & (mask_cur == 0)
    ys, xs, t = _t_values_for_mask_pixels(candidate.astype(np.uint8), normal, center)
    if xs.size <= 0:
        info.update({"reason": "no selected-mask difference candidates", "candidate_count": 0})
        return (mask_cur, depth_cur, info) if return_info else (mask_cur, depth_cur)

    pts = np.stack([xs.astype(np.float64), ys.astype(np.float64)], axis=1)
    s = (pts - center.reshape(1, 2)) @ axis
    protected = (t * float(protected_sign)) > 0.0
    target = (t * float(remove_sign)) > 0.0
    in_s = (s >= float(s_min_keep)) & (s <= float(s_max_keep))
    in_edge_band = (t >= float(t_band_min)) & (t <= float(t_band_max))

    # current maskに近い画素だけを戻す。edge bandだけでも十分だが，孤立したselected領域の復活を避けるため膨張近傍も見る。
    kernel = np.ones((5, 5), dtype=np.uint8)
    near_current = cv2.dilate(mask_cur.astype(np.uint8), kernel, iterations=2).astype(bool)
    near_current_vals = near_current[ys, xs]

    restore_bool = protected & in_s & in_edge_band & near_current_vals
    # 念のため target側は復活から完全排除する。
    restore_bool &= ~target

    raw_candidate_count = int(xs.size)
    protected_candidate_count = int(np.count_nonzero(protected))
    target_candidate_count = int(np.count_nonzero(target))
    after_s_count = int(np.count_nonzero(protected & in_s))
    after_band_count = int(np.count_nonzero(protected & in_s & in_edge_band))
    after_near_count = int(np.count_nonzero(restore_bool))

    if after_near_count <= 0:
        info.update({
            "reason": "no protected-side candidates after edge-band/near-current filter",
            "candidate_count": raw_candidate_count,
            "candidate_protected_count": protected_candidate_count,
            "candidate_target_count": target_candidate_count,
            "candidate_after_s_range_count": after_s_count,
            "candidate_after_edge_band_count": after_band_count,
            "candidate_after_near_current_count": after_near_count,
            "s_min_keep": float(s_min_keep),
            "s_max_keep": float(s_max_keep),
            "cur_t_min": float(cur_t_min),
            "cur_t_max": float(cur_t_max),
            "cur_t_p02": float(cur_t_p02),
            "cur_t_p98": float(cur_t_p98),
            "t_band_min": float(t_band_min),
            "t_band_max": float(t_band_max),
        })
        return (mask_cur, depth_cur, info) if return_info else (mask_cur, depth_cur)

    # 復活量の上限。多すぎる場合は protected 境界に近い画素から順に採用する。
    max_by_ratio = int(round(float(max_restore_area_ratio) * max(int(mask_cur.sum()), 1)))
    max_allowed = max(1, min(int(max_restore_pixels), max_by_ratio))
    restore_indices = np.where(restore_bool)[0]
    limited_by_count = False
    if restore_indices.size > max_allowed:
        if protected_sign < 0:
            # tが大きいほど現在境界に近い。
            order = np.argsort(-t[restore_indices])
        else:
            # tが小さいほど現在境界に近い。
            order = np.argsort(t[restore_indices])
        keep_idx = restore_indices[order[:max_allowed]]
        limited_restore_bool = np.zeros_like(restore_bool, dtype=bool)
        limited_restore_bool[keep_idx] = True
        restore_bool = limited_restore_bool
        limited_by_count = True

    restore_mask = np.zeros_like(mask_cur, dtype=np.uint8)
    restore_mask[ys[restore_bool], xs[restore_bool]] = 1
    restored_count = int(np.count_nonzero(restore_mask))

    if restored_count <= 0:
        info.update({
            "reason": "restore candidates became empty after max-count limit",
            "candidate_count": raw_candidate_count,
            "candidate_protected_count": protected_candidate_count,
            "candidate_target_count": target_candidate_count,
            "candidate_after_s_range_count": after_s_count,
            "candidate_after_edge_band_count": after_band_count,
            "candidate_after_near_current_count": after_near_count,
            "max_allowed_restore_pixels": int(max_allowed),
        })
        return (mask_cur, depth_cur, info) if return_info else (mask_cur, depth_cur)

    # 復活後の幅増加が大きすぎる場合は，境界に近いものだけに絞る。
    def _width_after(mask_test: np.ndarray):
        yy, xx = np.where(mask_test > 0)
        if xx.size < 20:
            return 0.0, None, None
        pp = np.stack([xx.astype(np.float64), yy.astype(np.float64)], axis=1)
        tt = (pp - center.reshape(1, 2)) @ normal
        p02, p98 = np.percentile(tt, [2.0, 98.0])
        return float(max(0.0, p98 - p02)), float(p02), float(p98)

    candidate_mask = ((mask_cur > 0) | (restore_mask > 0)).astype(np.uint8)
    cand_width, cand_p02, cand_p98 = _width_after(candidate_mask)
    width_guard_reverted = False
    width_limited = False
    if cand_width > cur_width_p02p98 + float(max_width_growth_px):
        # extension幅を段階的に縮め，幅ガードを満たす最大復活を探す。
        width_limited = True
        best_mask = None
        best_count = 0
        for frac in (0.75, 0.55, 0.40, 0.25, 0.12):
            ext = float(max_restore_extension_px) * float(frac)
            if protected_sign < 0:
                band_min = float(edge_t - ext)
                band_max = float(edge_t + float(inner_overlap_px))
            else:
                band_min = float(edge_t - float(inner_overlap_px))
                band_max = float(edge_t + ext)
            b = restore_bool & (t >= band_min) & (t <= band_max)
            rm = np.zeros_like(mask_cur, dtype=np.uint8)
            rm[ys[b], xs[b]] = 1
            mt = ((mask_cur > 0) | (rm > 0)).astype(np.uint8)
            w_after, _, _ = _width_after(mt)
            cnt = int(np.count_nonzero(rm))
            if cnt > 0 and w_after <= cur_width_p02p98 + float(max_width_growth_px):
                best_mask = rm
                best_count = cnt
                cand_width = w_after
                break
        if best_mask is not None:
            restore_mask = best_mask
            restored_count = int(best_count)
            candidate_mask = ((mask_cur > 0) | (restore_mask > 0)).astype(np.uint8)
            cand_width, cand_p02, cand_p98 = _width_after(candidate_mask)
        else:
            width_guard_reverted = True
            info.update({
                "used": False,
                "reason": "restore reverted by width-growth guard",
                "candidate_count": raw_candidate_count,
                "candidate_protected_count": protected_candidate_count,
                "candidate_target_count": target_candidate_count,
                "candidate_after_s_range_count": after_s_count,
                "candidate_after_edge_band_count": after_band_count,
                "candidate_after_near_current_count": after_near_count,
                "restored_count_before_width_guard": int(restored_count),
                "cur_width_p02p98_px": float(cur_width_p02p98),
                "candidate_width_p02p98_px": float(cand_width),
                "max_width_growth_px": float(max_width_growth_px),
                "width_guard_reverted": True,
                "s_min_keep": float(s_min_keep),
                "s_max_keep": float(s_max_keep),
    ...
    mask_out, depth_out, depth_fill_info = _restore_pixels_with_depth(
        mask01=mask_cur,
        depth_masked=depth_cur,
        restore_mask01=restore_mask,
        depth_source=None,
        fill_depth_by_current_row_median=True,
    )
    info.update({
        "used": True,
        "reason": "ok",
        ...
        "area_after_restore": int(mask_out.sum()),
        "area_growth_ratio": float(mask_out.sum() / max(int(mask_cur.sum()), 1)),
        "restore_limited_by_count": bool(limited_by_count),
        "max_allowed_restore_pixels": int(max_allowed),
        "width_limited": bool(width_limited),
        "width_guard_reverted": bool(width_guard_reverted),
        "cur_width_p02p98_px": float(cur_width_p02p98),
        "width_after_restore_p02p98_px": float(cand_width),
        "width_after_restore_p02": None if cand_p02 is None else float(cand_p02),
        "width_after_restore_p98": None if cand_p98 is None else float(cand_p98),
        "s_min_keep": float(s_min_keep),
        "s_max_keep": float(s_max_keep),
        "cur_t_min": float(cur_t_min),
        "cur_t_max": float(cur_t_max),
        ...
        "t_band_min": float(t_band_min),
        "t_band_max": float(t_band_max),
        "protected_side": state["protected_side"],
        "remove_side": state["remove_side"],
        "depth_fill": depth_fill_info,
    })

    if shot_dir is not None:
        try:
            debug_dir = Path(shot_dir) / "debug_one_sided_consistency"
            debug_dir.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(debug_dir / f"{stem}_restore_mask.png"), restore_mask.astype(np.uint8) * 255)
            save_json(debug_dir / f"{stem}_log.json", _jsonable_for_residual_diagnostics(info) if '_jsonable_for_residual_diagnostics' in globals() else info)
            if color_np is not None:
                overlay = np.asarray(color_np).copy()
                kept = (mask_cur > 0)
                overlay[kept] = (0, 255, 0)
                overlay[restore_mask > 0] = (255, 0, 255)
                blended = cv2.addWeighted(np.asarray(color_np), 0.65, overlay, 0.35, 0)
                cv2.imwrite(str(debug_dir / f"{stem}_overlay.png"), blended)
        except Exception as e:
            info["debug_save_error"] = str(e)

    return (mask_out, depth_out, info) if return_info else (mask_out, depth_out)

def restore_protected_side_removed_by_stage(
    *,
    before_mask01: np.ndarray,
    before_depth_masked: np.ndarray,
    after_mask01: np.ndarray,
    after_depth_masked: np.ndarray,
    column_info: dict | None,
    stage_name: str,
    shot_dir: str | Path | None = None,
    color_np: np.ndarray | None = None,
    stem: str = "protected_side_stage_guard",
    return_info: bool = True,
):
    """side判定後の各処理で protected_side 側が削られた場合，その画素だけを即時復活する。"""
    before_mask = (np.asarray(before_mask01) > 0).astype(np.uint8)
    after_mask = (np.asarray(after_mask01) > 0).astype(np.uint8)
    before_depth = np.asarray(before_depth_masked).copy()
    after_depth = np.asarray(after_depth_masked).copy()
    state, state_info = _extract_one_sided_state_from_column_info(column_info, require_used=True)
    info = {
        "enabled": True,
        "used": False,
        "reason": "not initialized",
        "algorithm": "restore_protected_side_removed_by_stage",
        "stage_name": str(stage_name),
        "side_state": state_info,
        "valid_count_before_stage": int(np.count_nonzero((before_mask > 0) & (before_depth > 0))),
        "valid_count_after_stage_before_restore": int(np.count_nonzero((after_mask > 0) & (after_depth > 0))),
    }
    if state is None:
        info["reason"] = "side state unavailable"
        return (after_mask, after_depth, info) if return_info else (after_mask, after_depth)

    removed = (before_mask > 0) & (before_depth > 0) & ~((after_mask > 0) & (after_depth > 0))
    ys, xs, t = _t_values_for_mask_pixels(removed.astype(np.uint8), state["normal"], state["center"])
    if xs.size <= 0:
        info.update({"reason": "no pixels removed by stage", "removed_count": 0})
        return (after_mask, after_depth, info) if return_info else (after_mask, after_depth)

    protected = (t * float(state["protected_sign"])) > 0.0
    target = (t * float(state["remove_sign"])) > 0.0
    restore_mask = np.zeros_like(after_mask, dtype=np.uint8)
    restore_mask[ys[protected], xs[protected]] = 1

    if int(np.count_nonzero(restore_mask)) <= 0:
        info.update({
            "reason": "stage removed only target/center side pixels",
            "removed_count": int(xs.size),
            "removed_target_side_count": int(np.count_nonzero(target)),
            "removed_protected_side_count": 0,
            "removed_t_min": float(np.min(t)),
            "removed_t_p50": float(np.median(t)),
            "removed_t_max": float(np.max(t)),
        })
        return (after_mask, after_depth, info) if return_info else (after_mask, after_depth)
    ...
    mask_out, depth_out, depth_fill_info = _restore_pixels_with_depth(
        mask01=after_mask,
        depth_masked=after_depth,
        restore_mask01=restore_mask,
        depth_source=before_depth,
        fill_depth_by_current_row_median=True,
    )
    info.update({
        "used": True,
        "reason": "protected-side pixels were removed by stage and restored",
        "removed_count": int(xs.size),
        "removed_target_side_count": int(np.count_nonzero(target)),
        "removed_protected_side_count": int(np.count_nonzero(protected)),
        "restored_count": int(np.count_nonzero(restore_mask)),
        "valid_count_after_stage_after_restore": int(np.count_nonzero((mask_out > 0) & (depth_out > 0))),
        "protected_side": state["protected_side"],
        "remove_side": state["remove_side"],
        "removed_t_min": float(np.min(t)),
        "removed_t_p05": float(np.percentile(t, 5.0)),
...
def _get_residual_policy_mode() -> str:
    ...
    if mode in {
        ...
        "AB_SAFE_A_TUNED_V3",
        "AB_SAFE_TUNED_V3",
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP",
        "AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP",
        "AB_SAFE_A_TUNED_V3_CLEAN_SKIP",
        "AB_SAFE_TUNED_V3_CLEAN_SKIP",
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE",
        "AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE",
        "AB_SAFE_A_TUNED_V3_SIDE_LOCK_RESTORE",
        "AB_SAFE_TUNED_V3_SIDE_LOCK_RESTORE",
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2",
        "AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2",
        "AB_SAFE_A_TUNED_V3_SIDE_LOCK_RESTORE_V2",
        "AB_SAFE_TUNED_V3_SIDE_LOCK_RESTORE_V2",
        "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3",
        "AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3",
        "AB_SAFE_A_TUNED_V3_DEPTH_SIDE_LOCK_V3",
        "AB_SAFE_TUNED_V3_DEPTH_SIDE_LOCK_V3",
    }:
        if mode == "ABSAFE":
            return "AB_SAFE"
        ...
        if mode == "AB_SAFE_TUNED_V3":
            return "AB_SAFE_A_TUNED_V3"
        if mode in {"AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_SKIP", "AB_SAFE_TUNED_V3_CLEAN_SKIP"}:
            return "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP"
        if mode in {"AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE", "AB_SAFE_A_TUNED_V3_SIDE_LOCK_RESTORE", "AB_SAFE_TUNED_V3_SIDE_LOCK_RESTORE"}:
            return "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE"
        if mode in {"AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2", "AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2", "AB_SAFE_A_TUNED_V3_SIDE_LOCK_RESTORE_V2", "AB_SAFE_TUNED_V3_SIDE_LOCK_RESTORE_V2"}:
            return "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2"
        if mode in {"AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3", "AB_SAFE_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3", "AB_SAFE_A_TUNED_V3_DEPTH_SIDE_LOCK_V3", "AB_SAFE_TUNED_V3_DEPTH_SIDE_LOCK_V3"}:
            return "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"
        return mode
    return "baseline"
...
def _residual_policy_a_variant(mode: str) -> str:
    mode = str(mode).upper()
    if mode in {"AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"}:
        return "a_tuned_v2"
    return "a_tuned" if mode == "AB_SAFE_A_TUNED" else "legacy"


def _residual_policy_is_v3_family(mode: str) -> bool:
    return str(mode).upper() in {"AB_SAFE_A_TUNED_V3", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"}


def _clean_rectangle_strong_skip_enabled(mode: str) -> bool:
    return str(mode).upper() in {"AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE_V2", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"}


def _depth_removed_ratio_from_prefilter(depth_prefilter_info: dict | None):
    if not isinstance(depth_prefilter_info, dict):
        return None
    detail = depth_prefilter_info.get("depth_filter_detail")
    if not isinstance(detail, dict):
        return None
    removed = _float_or_none(detail.get("removed_pixel_count"))
    remaining = _float_or_none(detail.get("remaining_pixel_count"))
    if removed is None or remaining is None:
        return None
    denom = float(removed + remaining)
    if denom <= 0.0:
        return None
    return float(removed / denom)


def evaluate_strong_clean_rectangle_skip_guard(
    shape_info: dict | None,
    depth_prefilter_info: dict | None,
    *,
    enabled: bool,
    width_cv_max: float = 0.03,
    width_max_over_median_max: float = 1.05,
    depth_removed_ratio_max: float = 0.005,
    require_needs_refine_false: bool = True,
) -> dict:
    """
    clean rectangle shadow candidate を止めるための強い安全ガード。

    背景:
      clean_rectangle は本来「形として長方形っぽい」という判定であり，
      側面がある証拠ではない。従来は clean rectangle を
      「追加削除しても安全そうな条件」として shadow candidate を評価していたが，
      側面が見えていない良好マスクでは過削りになることがある。

    このガードでは，
      - clean_rectangle=True
      - needs_refine=False
      - width_cv が極小
      - width_max_over_median が 1 に近い
      - Depth prefilter でほぼ削除なし
    のときだけ，clean rectangle shadow candidate / one-sided removal /
    post_profile_side_suppression を採用しない。

    注意:
      case番号，book名，GT幅は使わない。
      既存V3は変えず，新モード AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP でのみ有効。
    """
    shape = shape_info if isinstance(shape_info, dict) else {}
    depth_removed_ratio = _depth_removed_ratio_from_prefilter(depth_prefilter_info)
    width_cv = _float_or_none(shape.get("width_cv"))
    width_max_over_median = _float_or_none(shape.get("width_max_over_median"))

    checks = {
        "enabled": bool(enabled),
        "clean_rectangle": bool(shape.get("clean_rectangle", False)),
        "needs_refine_false": (not bool(shape.get("needs_refine", True))),
        "width_cv_ok": width_cv is not None and width_cv <= float(width_cv_max),
        "width_max_over_median_ok": width_max_over_median is not None and width_max_over_median <= float(width_max_over_median_max),
        "depth_removed_ratio_ok": depth_removed_ratio is not None and depth_removed_ratio <= float(depth_removed_ratio_max),
    }
    if not bool(require_needs_refine_false):
        checks["needs_refine_false"] = True

    reject_reasons = []
    if not checks["enabled"]:
        reject_reasons.append("mode_not_enabled")
    if not checks["clean_rectangle"]:
        reject_reasons.append("not_clean_rectangle")
    if not checks["needs_refine_false"]:
        reject_reasons.append("needs_refine_is_true")
    if not checks["width_cv_ok"]:
        reject_reasons.append(f"width_cv_not_strong_clean={width_cv}")
    if not checks["width_max_over_median_ok"]:
        reject_reasons.append(f"width_max_over_median_not_strong_clean={width_max_over_median}")
    if not checks["depth_removed_ratio_ok"]:
        reject_reasons.append(f"depth_removed_ratio_not_small={depth_removed_ratio}")

    triggered = bool(len(reject_reasons) == 0)
    return {
        "enabled": bool(enabled),
        "triggered": bool(triggered),
        "reason": "strong_clean_rectangle_skip_shadow_candidate" if triggered else "not_triggered",
        "reject_reasons": reject_reasons,
        "thresholds": {
            "width_cv_max": float(width_cv_max),
            "width_max_over_median_max": float(width_max_over_median_max),
            "depth_removed_ratio_max": float(depth_removed_ratio_max),
            "require_needs_refine_false": bool(require_needs_refine_false),
        },
        "observed": {
            "clean_rectangle": bool(shape.get("clean_rectangle", False)),
            "needs_refine": bool(shape.get("needs_refine", True)),
            "width_cv": width_cv,
            "width_max_over_median": width_max_over_median,
            "depth_removed_ratio": depth_removed_ratio,
            "depth_removed_pixel_count": (
                ((depth_prefilter_info or {}).get("depth_filter_detail") or {}).get("removed_pixel_count")
                if isinstance(depth_prefilter_info, dict) else None
            ),
            "depth_remaining_pixel_count": (
                ((depth_prefilter_info or {}).get("depth_filter_detail") or {}).get("remaining_pixel_count")
                if isinstance(depth_prefilter_info, dict) else None
            ),
        },
        "checks": checks,
...
def _evaluate_residual_policy_ab_safe_interaction(
    *,
    ...
    out = {
        "mode": mode,
        "enabled": mode in {"AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"},
    ...
    if mode not in {"AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE"}:
...
def _evaluate_residual_policy_b_width(
    *,
    ...
    out = {
        ...
        "v3_enabled": False,
        "v3_based_on": None,
        "v3_small_width_guard_candidate": False,
        "v3_small_width_guard_used": False,
        "v3_small_width_guard_blocked": False,
        "v3_small_width_guard_block_reason": None,
        "v3_small_width_guard_block_reasons": [],
        "v3_small_width_guard_proposal": None,
        "v3_current_width_mm": None,
        "v3_robust_width_mm": None,
        "v3_current_minus_robust_mm": None,
        "v3_current_over_robust_ratio": None,
        "v3_p2p98_width_mm": None,
        "v3_p10p90_width_mm": None,
        "v3_p2p98_minus_p10p90_mm": None,
        "v3_p2p98_over_p10p90_ratio": None,
        "v3_largest_component_ratio": None,
        "v3_keep_ratio": None,
        "v3_valid_keep_ratio": None,
    }
    if not enabled:
    ...
    tuned_v2_reject_reasons = []
    tuned_v2_kept = False
    if mode in {"AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP"} and reject_reasons and robust_width_mm is not None:
    ...
    v3_block_reasons = []
    v3_blocked = False
    v3_enabled = _residual_policy_is_v3_family(mode)
    if v3_enabled:
        out["v3_enabled"] = True
        out["v3_based_on"] = "AB_SAFE_A_TUNED_V2"
        out["v3_small_width_guard_proposal"] = "Proposal_C_block_non_improving_validation_band"
        out["v3_small_width_guard_candidate"] = bool(tuned_v2_kept)
        out["v3_current_width_mm"] = current_width_mm
        out["v3_robust_width_mm"] = robust_width_mm
        out["v3_current_minus_robust_mm"] = None if current_width_mm is None or robust_width_mm is None else float(current_width_mm - robust_width_mm)
        out["v3_current_over_robust_ratio"] = None if current_width_mm is None or robust_width_mm in (None, 0) else float(current_width_mm / robust_width_mm)
        out["v3_p2p98_width_mm"] = p2
        out["v3_p10p90_width_mm"] = p10
        out["v3_p2p98_minus_p10p90_mm"] = None if p2 is None or p10 is None else float(p2 - p10)
        out["v3_p2p98_over_p10p90_ratio"] = None if p2 is None or p10 in (None, 0) else float(p2 / p10)
        out["v3_largest_component_ratio"] = largest_ratio
        out["v3_keep_ratio"] = None
        out["v3_valid_keep_ratio"] = None

        if tuned_v2_kept:
            ratio = out["v3_p2p98_over_p10p90_ratio"]
            # Proposal C from the V2 small-width guard audit:
            # block over-shrink-prone bands without using case id, book name, or GT.
            if current_width_mm is not None and robust_width_mm is not None:
                if current_width_mm < 13.0 and robust_width_mm < 10.55:
                    v3_block_reasons.append(
                        f"proposal_c_low_current_low_robust=current={current_width_mm:.3f}mm robust={robust_width_mm:.3f}mm"
                    )
            else:
                v3_block_reasons.append("proposal_c_current_or_robust_width_unavailable")

            if current_width_mm is not None and ratio is not None:
                if 13.10 <= current_width_mm <= 13.25 and ratio < 1.21:
                    v3_block_reasons.append(
                        f"proposal_c_mid_current_low_ratio=current={current_width_mm:.3f}mm ratio={ratio:.3f}"
                    )
                if current_width_mm >= 13.4 and ratio > 1.232:
                    v3_block_reasons.append(
                        f"proposal_c_high_current_high_ratio=current={current_width_mm:.3f}mm ratio={ratio:.3f}"
                    )
            elif current_width_mm is None or ratio is None:
                v3_block_reasons.append("proposal_c_current_or_ratio_unavailable")

            if v3_block_reasons:
                v3_blocked = True
                tuned_v2_kept = False
                reject_reasons = list(tuned_v2_reject_reasons) + v3_block_reasons
                if not reject_reasons:
                    reject_reasons = v3_block_reasons

        out["v3_small_width_guard_blocked"] = bool(v3_blocked)
        out["v3_small_width_guard_block_reasons"] = v3_block_reasons
        out["v3_small_width_guard_block_reason"] = "; ".join(v3_block_reasons) if v3_block_reasons else None
        out["v3_small_width_guard_used"] = bool(tuned_v2_kept)
    ...
    if not reject_reasons and robust_width_mm is not None:
        ...
        out["triggered"] = True
        out["reason"] = (
            "robust width adopted by AB_SAFE_A_TUNED_V3 small-width stability guard"
            if v3_enabled and tuned_v2_kept
            else
...
def _run_recognition_core_like_offline(
    *,
    ...
    residual_policy_interaction_info = {
        ...
        "enabled": residual_policy_mode in {"AB_SAFE", "AB_SAFE_A_TUNED", "AB_SAFE_A_TUNED_V2", "AB_SAFE_A_TUNED_V3", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_SIDE_LOCK_RESTORE", "AB_SAFE_A_TUNED_V3_CLEAN_RECT_SKIP_DEPTH_SIDE_LOCK_V3"},
        "A_then_B_overwrite_prevented": False,
        "A_after_width_plausible": False,
        "A_after_width_sensitivity": None,
        "notes": ["not evaluated"],
    ...
    depth_prefilter_side_state = None
    depth_prefilter_side_state_info = {
        "used": False,
        "reason": "mode disabled",
        "algorithm": "infer_remove_side_from_depth_prefilter_removed_pixels",
    }
    if _depth_prefilter_side_lock_enabled(residual_policy_mode):
        depth_prefilter_side_state, depth_prefilter_side_state_info = infer_remove_side_from_depth_prefilter_removed_pixels(
            selected_mask01_before_depth=mask01_before_depth_filter,
            mask01_after_depth_prefilter=mask01,
            refine_info=depth_anchor_refine_info,
            image_shape=color_np.shape[:2],
            min_removed_pixels=300,
            dominant_fraction_min=0.70,
            dominant_ratio_min=1.50,
        )
        try:
            save_json(
                shot_dir / f"mask{sel_idx}{result_suffix}_depth_prefilter_side_lock_log.json",
                _jsonable_for_residual_diagnostics(depth_prefilter_side_state_info) if '_jsonable_for_residual_diagnostics' in globals() else depth_prefilter_side_state_info,
            )
        except Exception:
            pass
    ...
    depth_prefilter_info = {
        ...
        "depth_prefilter_removed_side_decision": depth_prefilter_side_state_info,
    ...
    strong_clean_rectangle_skip_info = evaluate_strong_clean_rectangle_skip_guard(
        shape_info=shape_info,
        depth_prefilter_info=depth_prefilter_info,
        enabled=_clean_rectangle_strong_skip_enabled(residual_policy_mode),
        width_cv_max=0.03,
        width_max_over_median_max=1.05,
        depth_removed_ratio_max=0.005,
        require_needs_refine_false=True,
    )
    shape_info["strong_clean_rectangle_skip_guard"] = strong_clean_rectangle_skip_info
    ...
    if needs_shape_refine:
        ...
        if bool(strong_clean_rectangle_skip_info.get("triggered", False)):
            print(
                f"[MASK SHAPE]{debug_prefix} strong clean rectangle guard triggered "
                f"({residual_policy_mode}) -> skip clean-rectangle shadow candidate"
            )
            clean_rectangle_shadow_info = {
                "evaluated": False,
                "reason": "strong clean rectangle guard triggered; shadow candidate skipped",
                "shadow_candidate_not_adopted_reason": "strong_clean_rectangle_guard",
                "strong_clean_rectangle_skip_guard": strong_clean_rectangle_skip_info,
                "shadow_candidate_adopted_as_final": False,
            }
            clean_rectangle_override_info = {
                "evaluated": True,
                "adopted": False,
                "reason": "candidate_rejected",
                "reject_reasons": ["strong_clean_rectangle_guard_triggered"],
                "adopt_reasons": [],
                "strong_clean_rectangle_skip_guard": strong_clean_rectangle_skip_info,
            }
            save_json(
    ...
    if needs_shape_refine:
        ...
        def _run_column_refine_candidate(use_guard: bool):
            return refine_mask_by_spine_column_length_after_depth(
                ...
                ocr_extended_protection_margin_px=0.0,
                ocr_extended_max_rescue_ratio=0.55,
                post_profile_side_suppression=not _one_sided_consistency_restore_enabled(residual_policy_mode),
                post_profile_width_margin=1.12,
                post_profile_max_shrink_ratio=0.22,
                force_remove_side=(depth_prefilter_side_state_info.get("remove_side") if _depth_prefilter_side_lock_enabled(residual_policy_mode) and isinstance(depth_prefilter_side_state_info, dict) and depth_prefilter_side_state_info.get("used") else None),
                forced_side_info=depth_prefilter_side_state_info if _depth_prefilter_side_lock_enabled(residual_policy_mode) else None,
                return_info=True,
            )
        ...
        column_info["depth_prefilter"] = depth_prefilter_info
        if _depth_prefilter_side_lock_enabled(residual_policy_mode):
            column_info["depth_prefilter_removed_side_decision"] = depth_prefilter_side_state_info
        if _one_sided_consistency_restore_enabled(residual_policy_mode):
            mask01_before_protected_restore = mask01.copy()
            depth_masked_before_protected_restore = depth_masked.copy()
            mask01, depth_masked, protected_restore_info = restore_protected_side_from_selected_mask_diff(
                current_mask01=mask01,
                current_depth_masked=depth_masked,
                selected_mask01_before_depth=mask01_before_depth_filter,
                raw_depth_u16=depth_np_u16,
                column_info=column_info,
                shot_dir=shot_dir,
                color_np=color_np,
                stem=f"mask{sel_idx}{result_suffix}_protected_side_restore_from_selected_diff",
                s_margin_px=18.0,
                return_info=True,
            )
            column_info["protected_side_restore_from_selected_diff"] = protected_restore_info
            save_spine_column_length_debug(
                shot_dir=shot_dir,
                color_np=color_np,
                mask_before=mask01_before_protected_restore,
                mask_after=mask01,
                depth_before=depth_masked_before_protected_restore,
                depth_after=depth_masked,
                column_info={
                    "used": bool(protected_restore_info.get("used", False)),
                    "reason": protected_restore_info.get("reason"),
                    "algorithm": "protected_side_restore_from_selected_diff",
                    "axis": column_info.get("axis"),
                    "normal": column_info.get("normal"),
                    "center": column_info.get("center"),
                    "selected_t_min": column_info.get("selected_t_min"),
                    "selected_t_max": column_info.get("selected_t_max"),
                    "protected_side_restore_from_selected_diff": protected_restore_info,
                },
                stem=f"mask{sel_idx}{result_suffix}_protected_side_restore_from_selected_diff",
            )
            _save_step_projection(
                "07b",
                "after_protected_side_restore_from_selected_diff",
                mask01,
                depth_masked,
                note="column refineで決めたprotected_sideについて，初期選択SAMマスクとの差分から判定以前に削られた点を復活．",
            )
        ...
        if _one_sided_consistency_restore_enabled(residual_policy_mode):
            mask01, depth_masked, final_t_width_clip_protected_restore_info = restore_protected_side_removed_by_stage(
                before_mask01=mask01_before_final_t_clip,
                before_depth_masked=depth_masked_before_final_t_clip,
                after_mask01=mask01,
                after_depth_masked=depth_masked,
                column_info=column_info,
                stage_name="final_t_width_clip",
                shot_dir=shot_dir,
                color_np=color_np,
                stem=f"mask{sel_idx}{result_suffix}_final_t_width_clip_protected_side_guard",
                return_info=True,
            )
            final_t_width_clip_info["protected_side_stage_restore"] = final_t_width_clip_protected_restore_info
        ...
        mask01, depth_masked, one_sided_column_prune_info = apply_one_sided_short_column_prune_after_final(
            ...
            small_component_area_ratio=0.0003,
            require_shape_not_worse=False,
            force_removed_side=column_info.get("remove_side") if _one_sided_consistency_restore_enabled(residual_policy_mode) else None,
            return_info=True,
        )
        ...
        if _one_sided_consistency_restore_enabled(residual_policy_mode):
            mask01, depth_masked, one_sided_column_prune_protected_restore_info = restore_protected_side_removed_by_stage(
                before_mask01=mask01_before_one_sided_column_prune,
                before_depth_masked=depth_masked_before_one_sided_column_prune,
                after_mask01=mask01,
                after_depth_masked=depth_masked,
                column_info=column_info,
                stage_name="one_sided_post_final_column_prune",
                shot_dir=shot_dir,
                color_np=color_np,
                stem=f"mask{sel_idx}{result_suffix}_one_sided_post_final_column_prune_protected_side_guard",
                return_info=True,
            )
            one_sided_column_prune_info["protected_side_stage_restore"] = one_sided_column_prune_protected_restore_info
    ...
    if _one_sided_consistency_restore_enabled(residual_policy_mode):
        mask01, depth_masked, ransac_spine_plane_protected_restore_info = restore_protected_side_removed_by_stage(
            before_mask01=mask01_before_ransac_plane,
            before_depth_masked=depth_masked_before_ransac_plane,
            after_mask01=mask01,
            after_depth_masked=depth_masked,
            column_info=column_info,
            stage_name="ransac_spine_plane",
            shot_dir=shot_dir,
            color_np=color_np,
            stem=f"mask{sel_idx}{result_suffix}_ransac_spine_plane_protected_side_guard",
            return_info=True,
        )
        ransac_spine_plane_info["protected_side_stage_restore"] = ransac_spine_plane_protected_restore_info
    ...
    if _one_sided_consistency_restore_enabled(residual_policy_mode):
        mask01, depth_masked, post_ransac_plane_a95_protected_restore_info = restore_protected_side_removed_by_stage(
            before_mask01=mask01_before_post_ransac_plane_a95,
            before_depth_masked=depth_masked_before_post_ransac_plane_a95,
            after_mask01=mask01,
            after_depth_masked=depth_masked,
            column_info=column_info,
            stage_name="post_ransac_plane_a95",
            shot_dir=shot_dir,
            color_np=color_np,
            stem=f"mask{sel_idx}{result_suffix}_post_ransac_plane_a95_protected_side_guard",
            return_info=True,
        )
        post_ransac_plane_a95_info["protected_side_stage_restore"] = post_ransac_plane_a95_protected_restore_info
    ...
    target_point_overlay_info = _draw_target_point_on_final_png(
        shot_dir=shot_dir,
        color_np=color_np,
        mask01=mask01,
        depth_masked=depth_masked,
    ...
    if isinstance(final_reconstruction_info, dict):
        final_reconstruction_info["target_point_overlay"] = target_point_overlay_info
    ...
    pca_json = {
        ...
        "book_width_info": book_width_info,
        "target_point_overlay_info": target_point_overlay_info,
    }
    ...
    processing_log = {
        ...
        "final_target_overlay_info": target_point_overlay_info,
