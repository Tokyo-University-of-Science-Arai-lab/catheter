...
def main():
    try:
        ...
        Xarm7.moveL_to_2nd_pos(asynchronous=True)
        ...
        print('after thresh check depth', PosGetter.c_p_right[2],
              PosGetter.c_p_left[2],
              PosGetter.c_p_top[2],
              PosGetter.c_p_bottom[2]) # depth の値チェック

        time.sleep(0.5)
        # culculate Xarm7 next_pose
        dy = PosGetter.get_dy()*1000 + (-1* DY_ADJ)
        print('dy : ', dy)
        ...
        ###########################################################
        print('Xarm7 moves')
        Xarm7.moveL_to_retrieval_pose(dy, d_roll, asynchronous=True)
        # gripper open
