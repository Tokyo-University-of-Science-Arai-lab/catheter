    ...
    try:
        ...
        try:
            ...
            for insert_attempt in range(1, MAX_CURRENT_INSERT_ATTEMPTS + 1):
                ...
                # X軸方向に引き抜いたその場でハンドを開き、本を離す。
                # (コンテナ収納は行わず、そのまま次の取り出しへ進む)
        ...
        # コンテナへは行かず撮影位置付近から戻るため、
        # 認識失敗時などと同じ capture_to_init を使う。
        waypoint_node.reset()
        waypoint_node.play_direct(
            config["paths"]["waypoint"]["capture_to_init"][side]
        )
        ...
        # 初期位置に戻った状態でオペレータの確認待ち。
        # Enterが押されるまで次の書籍の取り出しへは進まない。
        input("Enterキーを押すと次の書籍の取り出しに進みます...")
